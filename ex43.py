	...
	def enter(self):
		...
		if action == "shoot!":
			print(dedent("""
			      ...
			      which throws off your aim. Your laser hits his costume but 
			      misses him entirely. This completely ruins his brand new
			...
			print(dedent("""
			      ...
			      jura fur fvgf nebhaq gur ubhfr, fur fvgf nebhaq gur ubhfr. The
			      Gothon stops, tries not to laugh, then busts out laughing and
			      can't move. While he's laughing you run up and shoot him square
			      in the head putting him down, then jump through the Weapon 
			      Armory door.
			      """))
...
class LaserWeaponArmory(Scene):
	...
	def enter(self):
		print(dedent("""
			...
			the lock closes forever and you can't get the bomb. The code is 3
			digits.
			"""))
		...
		if guess == code:
			print(dedent("""
				...
				out. You grab the neutron bomb and run as fast as yu can to
...
class TheBridge(Scene):
	...
	def enter(self):
		print(dedent("""
			...
			arm and surprise 5 Gothons who are trying to take control of the 
			ship. Each of them has an even uglier clown costume than the last.
		...
		if action == "throw the bomb":
			print(dedent("""
				...
				shoots you right in the back killing you. As you die you see
				another Gothon frantically try to disarm the bomb. You die
...
class EscapePod(Scene):
	...
	def enter(self):
		print(dedent("""
			...
			one to take. Some of them could be damaged but you don't have time
			to look. There's 5 pods, which one do you take?
			"""))
		...
		if int(guess) != good_pod:
			...
			print(dedent(f"""
				You jump into pod {guess} and hit the eject button. The pod
				easily slides out into space heading to the planet below. As
				it flies to the planet, you look back and see your ship
